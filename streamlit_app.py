...
def knez_admin_ui():
    ...
    with _kt3:
        ...
        if _rez.get("izv"):
            ...
            # ---------- 1) Ista pumpa poslala VIŠE fajlova ----------
            # Ako su fajlovi isti — uzima se jedan. Ako NISU — to je problem i mora
            # da se odluči koji važi (inače bi stanje te pumpe bilo pogrešno).
            _po_pumpi = {}
            for _x in _izv:
                if _x.get("idk") and _x.get("redova"):
                    _po_pumpi.setdefault(int(_x["idk"]), []).append(_x)
            _dupli_isti, _dupli_razl = {}, {}
            ...
            for _x in _izv:
                _ik = _x.get("idk")
                if not _x.get("redova"):
                    _x["st_ikona"] = "✗"
                    _x["st_txt"] = "nije pročitan — " + (str(_x.get("greska", ""))[:70]
                                                         or "nepoznat obrazac")
                elif not _ik:
                    _x["st_ikona"] = "⚠"
                    _x["st_txt"] = "pumpa nije prepoznata (BS " + str(_x.get("bs") or "?") + ")"
                elif not _x.get("per_ok"):
                    _x["st_ikona"] = "⚠"
                    ...
                    _x["st_txt"] = ("dva ista fajla — uzet jedan" if _uzmi.get(int(_ik)) == _x["ime"]
                                    else "duplikat, preskočen")
                else:
                    _x["st_ikona"] = "✓"
                    _x["st_txt"] = "OK"
            _n_ok = sum(1 for _x in _izv if _x["st_ikona"] == "✓")
            _prob = [_x for _x in _izv if _x["st_ikona"] != "✓"]
            ...
            # ---------- 6) Artikli: ručne ispravke i nepoznati ----------
            ...
            _rucne_st = []
            for r in _pumpe:
                _rl = ((_obr.get(int(r["idk"])) or {}).get("dnevnik") or {}).get("rucni_lager") or {}
                for _x in (_rl.get("redovi") or []):
                    _rucne_st.append({"idk": int(r["idk"]), "pumpa": r["naziv"], "bs": "",
                                      "sifra": "", "naziv": _x.get("naziv", ""),
                                      "ida": _x.get("ida"), "nas_naziv": _x.get("naziv", ""),
                                      "poklapanje": "ručno upisano",
                                      "izlaz": _x.get("izlaz"), "stanje": _x.get("stanje"),
            ...
            for r in _pumpe:
                ...
                elif not _f.get("per_ok"):
                    _raz = "parametri nisu kako treba — " + str(_f.get("per_txt", ""))
                else:
                    _raz = "nije ušla u tabelu"
                _za_rucno.append({"idk": _ik, "naziv": r["naziv"], "razlog": _raz})
            st.markdown("<div style='height:10px;'></div>", unsafe_allow_html=True)
            st.markdown('<div style="font-size:14px;font-weight:800;color:#7c2d12;">'
                        '🛠 Pumpe bez izveštaja ili sa problemom (' + str(len(_za_rucno))
                        + ')</div><div style="font-size:12px;color:#9a3412;margin:2px 0 8px;">'
                        'Klikni na pumpu pa upiši prodaju (izlaz) i lager (stanje) — čim sačuvaš, '
            ...
            _n_rucnih = len([s for s in _spremno if s.get("poklapanje") == "ručno upisano"])
            st.markdown("<div style='height:8px;'></div>", unsafe_allow_html=True)
            st.caption("U Excel ide " + str(len(_spremno)) + " redova  ·  pumpi: "
                       + str(len({s["idk"] for s in _spremno})) + "  ·  sa našim ID artikla: "
                       + str(len(_sa_art))
                       + ("  ·  ručno upisano: " + str(_n_rucnih) if _n_rucnih else ""))
            if _spremno:
                try:
...
def sb_prikup_sadrzaj_set(mesec_key, sistem, prodaja, lager_obj, lager_uk, ko=""):
    """Šta izveštaj tog sistema sadrži — upisuje se posle pregleda priloga.
    prodaja: jedna od PRIKUP_PRODAJA; lager_obj/lager_uk: True/False."""
    cli, _dn = _prikup_dnevnik(mesec_key, sistem)
    if cli is None:
        return False
    try:
        _dn["sadrzaj"] = {"prodaja": str(prodaja or PRIKUP_PRODAJA[0]),
                          "lager_obj": bool(lager_obj), "lager_uk": bool(lager_uk),
                          "ko": str(ko or ""), "at": _now().isoformat()}
        _prikup_upisi(cli, mesec_key, sistem, _dn)
        return True
    except Exception:
...
def _prikup_sadrzaj_kratko(sadrzaj):
    """Kratak opis šta izveštaj sadrži — za naslov kartice sistema."""
    _s = sadrzaj or {}
    _p = str(_s.get("prodaja") or "")
    if not _s.get("at"):
        return ""
    _d = []
    if _p == "po objektima":
        _d.append("prodaja po objektima")
    elif _p == "samo ukupna prodaja":
        _d.append("samo ukupna prodaja")
    elif _p == "nema prodaje":
        _d.append("bez prodaje")
    if _s.get("lager_obj"):
        _d.append("lager po objektu")
    if _s.get("lager_uk"):
        _d.append("ukupan lager")
    return ", ".join(_d) if _d else "ništa od traženog"
...
def prikup_admin_ui():
    ...
    st.markdown(
        ...
        '<div style="background:#dcfce7;border:1px solid #86efac;border-radius:12px;padding:15px 18px;">'
        '<div style="font-size:22px;font-weight:800;color:#14532d;">' + str(_n_odg) + '</div>'
        '<div style="font-size:12px;color:#166534;margin-top:3px;font-weight:700;">'
        '📎 Stigao izveštaj</div></div>'
        '<div style="background:' + ("#fffbeb" if _n_ceka else "#f8fafc")
        + ';border:1px solid ' + ("#fde68a" if _n_ceka else "#e5e7eb")
    ...
    for s in _sistemi:
        ...
        with st.expander(s + "   —   " + _zn, expanded=False):
            ...
            if _ima_fajl:
                st.markdown('<div style="margin:14px 0 2px;font-size:12px;font-weight:700;'
                            'color:#5b21b6;">🔎 Šta izveštaj sadrži</div>',
                            unsafe_allow_html=True)
                st.caption("Otvori prilog, pogledaj šta je poslato i štikliraj. Ostaje "
                           "zapamćeno za „" + _h_escape(_sel_lbl) + "“."
                ...
                with st.form("prikup_sadr_" + s, border=False):
                    ...
                    with _sc_b:
                        ...
                        st.markdown("<div style='font-size:14px;font-weight:600;"
                                    "color:#31333f;margin-bottom:6px;'>Stanje zaliha</div>",
                                    unsafe_allow_html=True)
...
BARKOD_UGRADJEN = {
    1: "8600572835028", 2: "8600572835035", 3: "8600572835042", 4: "8600572835059",
    5: "8600572835066", 6: "8600572835073", 7: "8600572835080", 8: "8600572835097",
    9: "8600572835103", 10: "8600572835110", 11: "8600572835127", 12: "8600572835134",
    13: "8600572835141", 14: "8600572835158", 15: "8600572835165", 16: "8600572835172",
    17: "8600572835189", 18: "8600572835196", 19: "8600572835202", 20: "8600572835219",
    21: "8600572835226", 22: "8600572835233", 23: "8600572835240", 24: "8600572835257",
    25: "8600572835264", 26: "8600572835271", 27: "8600572835288", 28: "8600572835295",
    29: "8600572835301", 30: "8600572835318", 31: "8600572835325", 32: "8600572835332",
    33: "8600572835349", 34: "8600572835356", 35: "8600572835363", 36: "8600572835370",
    37: "8600572835387", 38: "8600572835394", 39: "8600572835400", 40: "8600572835417",
    688: "8606031996347", 689: "8606031996354", 690: "8606031996361", 691: "8606031996378",
    692: "8606031996385", 693: "8606031996392", 694: "8606031996408", 695: "8606031996415",
    696: "8606031996422", 697: "8606031996439", 698: "8606031996446", 699: "8606031996453",
    700: "8606031996460", 701: "8606031996477", 702: "8606031996484", 703: "8606031996491",
    704: "8606031996507", 705: "8606031996514", 706: "8606031996521", 707: "8606031996538",
    708: "8606031996545", 709: "8606031996552", 710: "8606031996569", 711: "8606031996576",
    712: "8606031996583", 713: "8606031996590", 714: "8606031996606", 715: "8606031996613",
    716: "8606031996620", 717: "8606031996637", 718: "8606031996644", 719: "8606031996651",
    720: "8606031996668", 721: "8606031996675", 722: "8606031996682", 723: "8606031996699",
    724: "8606031996705", 725: "8606031996712", 726: "8606031996729", 727: "6923742034123",
    728: "6923742034109", 729: "6923742034345", 730: "6923742034260", 731: "6923742034246",
    732: "6923742034062", 733: "6923742034000", 734: "6923742034130", 735: "6923742034116",
    736: "6923742034352", 737: "6923742034277", 738: "6923742034253", 739: "6923742034079",
    740: "6923742034017", 741: "6923742040216", 742: "6923742040193", 743: "6923742040179",
    744: "6923742040155", 745: "6923742040131", 746: "6923742040117", 747: "6923742040094",
    748: "6923742040070", 749: "6923742040056", 750: "6923742040032", 751: "6923742040018",
    752: "6923742039999", 753: "6923742039975", 754: "6923742039951", 755: "6923742039937",
    756: "6923742040223", 757: "6923742040209", 758: "6923742040186", 759: "6923742040162",
    760: "6923742040148", 761: "6923742040124", 762: "6923742040100", 763: "6923742040087",
    764: "6923742040063", 765: "6923742040049", 766: "6923742040025", 767: "6923742040001",
    768: "6923742039982", 769: "6923742039968", 770: "6923742039944",
}
...
def _knez_dat(s):
    """'1.8.2026' / '01.08.2026.' -> datetime.date (ili None)."""
    import re as _r
    _m = _r.match(r"\s*(\d{1,2})\.\s*(\d{1,2})\.\s*(\d{2,4})", str(s or ""))
    if not _m:
        return None
    try:
        _g = int(_m.group(3))
        if _g < 100:
            _g += 2000
        return datetime.date(_g, int(_m.group(2)), int(_m.group(1)))
    except Exception:
        return None


def _knez_period_status(od, do, mesec_key):
    """Da li je lager lista napravljena za TAJ mesec (od 01. do poslednjeg u mesecu).
    Vraća (ok, kratak tekst za kolonu Status)."""
    import calendar as _cal
    try:
        _g = int(str(mesec_key).split("-")[0]); _m = int(str(mesec_key).split("-")[1])
    except Exception:
        return (True, "")
    _tr_od = datetime.date(_g, _m, 1)
    _tr_do = datetime.date(_g, _m, _cal.monthrange(_g, _m)[1])
    _d1, _d2 = _knez_dat(od), _knez_dat(do)
    if not _d1 or not _d2:
        return (False, "nema perioda u fajlu")
    if _d1 == _tr_od and _d2 == _tr_do:
        return (True, "")
    return (False, "period " + _d1.strftime("%d.%m.%Y") + "–" + _d2.strftime("%d.%m.%Y")
            + ", a treba " + _tr_od.strftime("%d.%m.") + "–" + _tr_do.strftime("%d.%m.%Y"))
...
def sb_knez_rucni_set(mesec_key, idk, redovi, ko="", prazno=False):
    ...
    try:
        ...
        cli.table("obrada").upsert(
            {"mesec": mesec_key, "sistem": KNEZ_SIS, "idk": int(idk),
             "reakcije": list(_r.get("reakcije") or []),
