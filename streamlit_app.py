...
def create_pdf(engine, period_str, n_mes):
    buf = io.BytesIO()
    ...
    s_title = sty('title', fontSize=22, fontName='Helvetica-Bold', textColor=colors.HexColor('#111111'), spaceAfter=2*mm)
    s_subtitle = sty('sub', fontSize=10, fontName='Helvetica', textColor=colors.HexColor('#888888'), spaceAfter=6*mm)
    s_section = sty('sec', fontSize=13, fontName='Helvetica-Bold', textColor=colors.HexColor('#3b82f6'), spaceBefore=5*mm, spaceAfter=3*mm)
    s_body = sty('body', fontSize=9.5, fontName='Helvetica', textColor=colors.HexColor('#333333'), leading=15, spaceAfter=3*mm)
    s_small = sty('small', fontSize=8, fontName='Helvetica', textColor=colors.HexColor('#888888'), leading=12, spaceAfter=2*mm)
    s_bold = sty('bold', fontSize=9.5, fontName='Helvetica-Bold', textColor=colors.HexColor('#111111'), leading=15)

    prof = engine.df_profit_obj.copy()
    oos_ukupno = int(engine.df_oos['Izgubljeni_profit'].sum()) if len(engine.df_oos) > 0 else 0
    ...
    mes_oos = oos_ukupno // max(n_mes, 1)

    ukupno_obj = len(prof)
    n_neto_neg = int((prof['Neto_profit'] <= 0).sum())
    n_oos_neg = int(((prof['Neto_profit'] <= 0) & (prof['Potencijalni_profit'] > 0)).sum())
    n_pravi_neg = int(((prof['Neto_profit'] <= 0) & (prof['Potencijalni_profit'] <= 0)).sum())
    ...
    usteda_gubitak = abs(prof[prof['Neto_profit'] <= 0]['Neto_profit'].sum())

    pozitivni = prof[prof['Potencijalni_profit'] > 0]
    neto_pozitivnih = int(pozitivni['Neto_profit'].sum())
    pravi_neg_df = prof[(prof['Neto_profit'] <= 0) & (prof['Potencijalni_profit'] <= 0)]
    usteda_trosak2 = int(n_pravi_neg * trosak_po_obj)
    usteda_gubitak2 = int(abs(pravi_neg_df['Neto_profit'].sum()))
    ukupni_potencijal = neto_pozitivnih + usteda_trosak2 + usteda_gubitak2 + oos_ukupno
    stvarni_neto = int(prof['Neto_profit'].sum())

    story = []

    # --- Naslov ---
    story.append(Paragraph("Analiza profitabilnosti <font color='#3b82f6'>mreže objekata</font>", s_title))
    story.append(Paragraph(f"Finansijski pregled · {period_str} · {ukupno_obj} objekata · {n_mes} meseci", s_subtitle))
    story.append(HRFlowable(width=W, thickness=1, color=colors.HexColor('#e5e7eb'), spaceAfter=5*mm))

    # --- 4 summary kartice kao tabela ---
    def kard_cell(label, total, mes, color_hex):
        return f"<b><font color='{color_hex}' size='13'>{total:,} RSD</font></b><br/><font color='#888888' size='8'>{mes:,} RSD / mesec</font>"

    kard_data = [
        [Paragraph(f"<font color='#999999' size='7'>UKUPAN TROSAK · {n_mes} MESECI</font><br/>{kard_cell('', total_trosak, mes_trosak, '#f59e0b')}", ss['Normal']),
         Paragraph(f"<font color='#999999' size='7'>BRUTO PROFIT · {n_mes} MESECI</font><br/>{kard_cell('', total_bruto, mes_bruto, '#10b981')}", ss['Normal']),
         Paragraph(f"<font color='#999999' size='7'>NETO PROFIT · {n_mes} MESECI</font><br/>{kard_cell('', total_neto, mes_neto, '#3b82f6' if total_neto >= 0 else '#ef4444')}", ss['Normal']),
         Paragraph(f"<font color='#999999' size='7'>OOS IZGUBLJEN · {n_mes} MESECI</font><br/>{kard_cell('', oos_ukupno, mes_oos, '#ef4444')}", ss['Normal'])],
    ]
    kw = W / 4 - 2*mm
    kard_tbl = Table(kard_data, colWidths=[kw]*4, rowHeights=[18*mm])
    kard_tbl.setStyle(TableStyle([
        ('BOX', (0,0),(0,0), 1, colors.HexColor('#f59e0b')),
        ('BOX', (1,0),(1,0), 1, colors.HexColor('#10b981')),
        ('BOX', (2,0),(2,0), 1, colors.HexColor('#3b82f6' if total_neto >= 0 else '#ef4444')),
        ('BOX', (3,0),(3,0), 1, colors.HexColor('#ef4444')),
        ('LEFTPADDING', (0,0),(-1,-1), 6), ('RIGHTPADDING', (0,0),(-1,-1), 6),
        ('TOPPADDING', (0,0),(-1,-1), 5), ('BOTTOMPADDING', (0,0),(-1,-1), 5),
        ('VALIGN', (0,0),(-1,-1), 'MIDDLE'),
    ]))
    story.append(kard_tbl)
    story.append(Spacer(1, 5*mm))

    # --- 1. Mesecni trend ---
    story.append(Paragraph("1 · Mesečni trend bruto profita", s_section))
    a_labels = engine.analitika_labels if engine.analitika_labels else engine.mesec_labels
    bruto_po_mes = [(lb, int(prof[f'Bruto_{lb}'].sum()) if f'Bruto_{lb}' in prof.columns else 0) for lb in a_labels]
    neto_po_mes  = [(lb, int(prof[f'Neto_{lb}'].sum()) if f'Neto_{lb}' in prof.columns else 0) for lb in a_labels]

    def bar_rows(data, color_pos, color_neg):
        max_v = max(abs(v) for _, v in data) if data else 1
        rows = []
        for lb, v in data:
            bar_w = abs(v) / max_v * 90  # mm
            c = color_pos if v >= 0 else color_neg
            rows.append([
                Paragraph(f"<font size='7' color='#888888'>{lb}</font>", ss['Normal']),
                Table([['']], colWidths=[bar_w*mm], rowHeights=[4*mm],
                    style=[('BACKGROUND',(0,0),(0,0),colors.HexColor(c)),('BOX',(0,0),(0,0),0,colors.white)]),
                Paragraph(f"<font size='7' color='#555555'>{v:,} RSD</font>", ss['Normal']),
            ])
        return rows

    br = bar_rows(bruto_po_mes, '#10b981', '#ef4444')
    if br:
        bt = Table(br, colWidths=[18*mm, 95*mm, 35*mm])
        bt.setStyle(TableStyle([('VALIGN',(0,0),(-1,-1),'MIDDLE'),('TOPPADDING',(0,0),(-1,-1),1),('BOTTOMPADDING',(0,0),(-1,-1),1)]))
        story.append(bt)

    story.append(Spacer(1, 3*mm))
    story.append(Paragraph("2 · Mesečni trend neto profita", s_section))
    nr = bar_rows(neto_po_mes, '#3b82f6', '#ef4444')
    if nr:
        nt = Table(nr, colWidths=[18*mm, 95*mm, 35*mm])
        nt.setStyle(TableStyle([('VALIGN',(0,0),(-1,-1),'MIDDLE'),('TOPPADDING',(0,0),(-1,-1),1),('BOTTOMPADDING',(0,0),(-1,-1),1)]))
        story.append(nt)

    story.append(Spacer(1, 4*mm))

    # --- 2. Profitabilnost po objektima ---
    story.append(HRFlowable(width=W, thickness=0.5, color=colors.HexColor('#e5e7eb'), spaceAfter=3*mm))
    story.append(Paragraph("3 · Profitabilnost po objektima", s_section))
    story.append(Paragraph(
        f"Od <b>{ukupno_obj} objekata</b>, <b>{n_neto_neg}</b> je neto negativno. Medjutim, <b>{n_oos_neg}</b> od njih ima negativan neto "
        f"isključivo zbog OOS-a — kada se uračuna izgubljena zarada, njihov potencijal je pozitivan. Ovi objekti nisu problem, samo nisu imali robu.",
        s_body))
    story.append(Paragraph(
        f"Pravih neprofitabilnih je <b>{n_pravi_neg}</b> ({pct_pravi}% ukupne mreže) — negativni čak i po potencijalu. "
        f"Trošak po objektu je <b>{trosak_po_obj:,.0f} RSD</b> za {n_mes} meseci / <b>{trosak_mes_obj:,.0f} RSD</b> mesečno.",
        s_body))
    story.append(Paragraph(
        f"Zatvaranjem <b>{n_pravi_neg} pravih neprofitabilnih</b> skidamo trošak "
        f"<b>{n_pravi_neg} × {trosak_po_obj:,.0f} RSD = {usteda_trosak:,.0f} RSD</b> "
        f"({usteda_trosak/max(n_mes,1):,.0f} RSD/mes) i prestajemo da gubimo "
        f"<b>{usteda_gubitak:,.0f} RSD</b> ({usteda_gubitak/max(n_mes,1):,.0f} RSD/mes) na negativnim objektima.",
        s_body))

    # --- 3. OOS ---
    story.append(HRFlowable(width=W, thickness=0.5, color=colors.HexColor('#e5e7eb'), spaceAfter=3*mm))
    story.append(Paragraph("4 · Izgubljena zarada zbog nedostatka robe (OOS)", s_section))
    oos_sum_data = [
        [Paragraph(f"<b><font color='#ef4444' size='13'>{oos_ukupno:,} RSD</font></b><br/><font color='#888' size='7'>Izgubljen profit ({n_mes}m)</font>", ss['Normal']),
         Paragraph(f"<b><font color='#ef4444' size='13'>{mes_oos:,} RSD</font></b><br/><font color='#888' size='7'>Prosečno mesečno</font>", ss['Normal']),
         Paragraph(f"<b><font color='#ef4444' size='13'>{int((engine.df_oos.get('Lager_danas',pd.Series([0]))==0).sum()) if len(engine.df_oos)>0 else 0:,}</font></b><br/><font color='#888' size='7'>Kombinacija na 0 lagera danas</font>", ss['Normal'])],
    ]
    oos_t = Table(oos_sum_data, colWidths=[W/3]*3, rowHeights=[14*mm])
    oos_t.setStyle(TableStyle([
        ('TOPBORDER',(0,0),(0,0),2,colors.HexColor('#ef4444')),
        ('TOPBORDER',(1,0),(1,0),2,colors.HexColor('#ef4444')),
        ('TOPBORDER',(2,0),(2,0),2,colors.HexColor('#ef4444')),
        ('LEFTPADDING',(0,0),(-1,-1),6),('RIGHTPADDING',(0,0),(-1,-1),6),
        ('TOPPADDING',(0,0),(-1,-1),5),('BOTTOMPADDING',(0,0),(-1,-1),5),
        ('VALIGN',(0,0),(-1,-1),'MIDDLE'),
        ('LINEABOVE',(0,0),(0,0),2,colors.HexColor('#ef4444')),
        ('LINEABOVE',(1,0),(1,0),2,colors.HexColor('#ef4444')),
        ('LINEABOVE',(2,0),(2,0),2,colors.HexColor('#ef4444')),
    ]))
    story.append(oos_t)
    ...
    if len(engine.df_oos) > 0:
        story.append(Spacer(1, 3*mm))
        story.append(Paragraph("<b>Top 5 artikala po izgubljenom profitu:</b>", s_bold))
        oos_art5 = engine.df_oos.groupby(['id artikla','Naziv artikla']).agg(
            Izgubljeni_profit=('Izgubljeni_profit','sum')).reset_index().sort_values('Izgubljeni_profit',ascending=False).head(5)
        top5_max = int(oos_art5['Izgubljeni_profit'].max())
        bar_colors_5 = ['#ef4444','#f97316','#ec4899','#8b5cf6','#3b82f6']
        top5_rows = []
        for i, (_, row) in enumerate(oos_art5.iterrows()):
            naziv = str(row['Naziv artikla'])[:40]
            val = int(row['Izgubljeni_profit'])
            bw = val / top5_max * 80
            c = bar_colors_5[i % 5]
            top5_rows.append([
                Paragraph(f"<font size='8' color='#444'>{naziv}</font>", ss['Normal']),
                Table([['']], colWidths=[bw*mm], rowHeights=[4*mm],
                    style=[('BACKGROUND',(0,0),(0,0),colors.HexColor(c)),('BOX',(0,0),(0,0),0,colors.white)]),
                Paragraph(f"<font size='8' color='{c}'><b>{val:,} RSD</b></font>", ss['Normal']),
            ])
        t5t = Table(top5_rows, colWidths=[52*mm, 85*mm, 28*mm])
        t5t.setStyle(TableStyle([('VALIGN',(0,0),(-1,-1),'MIDDLE'),('TOPPADDING',(0,0),(-1,-1),2),('BOTTOMPADDING',(0,0),(-1,-1),2)]))
        story.append(t5t)

    # --- 4. Scenario ---
    story.append(HRFlowable(width=W, thickness=0.5, color=colors.HexColor('#e5e7eb'), spaceAfter=3*mm))
    story.append(Paragraph("5 · Scenario: Optimalna mreža", s_section))
    story.append(Paragraph(
        f"Ako se istovremeno zatvore neprofitabilni objekti i eliminiše OOS, mreža ide sa <b>{stvarni_neto:,} RSD</b> neto profita na "
        f"<b><font color='#10b981'>+{ukupni_potencijal:,} RSD</font></b> za {n_mes} meseci.", s_body))

    sc_rows = [
        [Paragraph(f"<font size='9' color='#555'>Neto profit pozitivnih objekata (potencijal > 0)</font>", ss['Normal']),
         Paragraph(f"<font color='#10b981'><b>+{neto_pozitivnih:,} RSD</b></font>", ss['Normal'])],
        [Paragraph(f"<font size='9' color='#555'>Ušteda: zatvaranje {n_pravi_neg} neprofitabilnih obj.</font>", ss['Normal']),
         Paragraph(f"<font color='#10b981'><b>+{usteda_trosak2+usteda_gubitak2:,} RSD</b></font>", ss['Normal'])],
        [Paragraph(f"<font size='9' color='#555'>Povraćaj izgub. zarade (OOS eliminacija)</font>", ss['Normal']),
         Paragraph(f"<font color='#10b981'><b>+{oos_ukupno:,} RSD</b></font>", ss['Normal'])],
        [Paragraph(f"<b>UKUPNI POTENCIJAL ({n_mes} meseci)</b>", ss['Normal']),
         Paragraph(f"<b><font color='#10b981'>= {ukupni_potencijal:,} RSD</font></b>", ss['Normal'])],
        [Paragraph(f"<font size='9' color='#555'>Stvarni neto profit ({n_mes} meseci)</font>", ss['Normal']),
         Paragraph(f"<font color='#555'>{stvarni_neto:,} RSD</font>", ss['Normal'])],
        [Paragraph(f"<font size='9' color='#8b5cf6'>Razlika — potencijal koji još nije ostvaren</font>", ss['Normal']),
         Paragraph(f"<font color='#8b5cf6'><b>+{ukupni_potencijal-stvarni_neto:,} RSD</b></font>", ss['Normal'])],
    ]
    sc_tbl = Table(sc_rows, colWidths=[W*0.72, W*0.28])
    sc_tbl.setStyle(TableStyle([
        ('LINEBELOW',(0,0),(-1,2),0.5,colors.HexColor('#f3f4f6')),
        ('LINEABOVE',(0,3),(-1,3),1.5,colors.HexColor('#e5e7eb')),
        ('LINEBELOW',(0,3),(-1,3),1.5,colors.HexColor('#e5e7eb')),
        ('BACKGROUND',(0,3),(-1,3),colors.HexColor('#f0fdf4')),
        ('TOPPADDING',(0,0),(-1,-1),4),('BOTTOMPADDING',(0,0),(-1,-1),4),
        ('LEFTPADDING',(0,0),(-1,-1),4),('RIGHTPADDING',(1,0),(1,-1),4),
        ('ALIGN',(1,0),(1,-1),'RIGHT'),
    ]))
    story.append(sc_tbl)

    # --- 5. Tabela po okruzima ---
    if engine.region_map and len(prof) > 0:
        story.append(HRFlowable(width=W, thickness=0.5, color=colors.HexColor('#e5e7eb'), spaceAfter=3*mm))
        story.append(Paragraph("6 · Profitabilnost po okruzima", s_section))
        prof_reg = prof.copy()
        prof_reg['Region'] = prof_reg['ID KOMITENTA'].map(engine.region_map).fillna('Ostalo')
        prof_reg['Profitabilan'] = prof_reg['Neto_profit'] > 0
        reg_grp = prof_reg.groupby('Region').agg(Ukupno=('ID KOMITENTA','count'), Ostaje=('Profitabilan','sum')).reset_index()
        reg_grp['Zatvara'] = reg_grp['Ukupno'] - reg_grp['Ostaje']
        reg_grp = reg_grp.sort_values('Ukupno', ascending=False).reset_index(drop=True)
        mali_okruzi = reg_grp[reg_grp['Ostaje'] < 5]['Region'].tolist()

        reg_header = [
            Paragraph("<b><font size='7' color='#888'>OKRUG</font></b>", ss['Normal']),
            Paragraph("<b><font size='7' color='#888'>UKUPNO</font></b>", ss['Normal']),
            Paragraph("<b><font size='7' color='#10b981'>✓ OSTAJE</font></b>", ss['Normal']),
            Paragraph("<b><font size='7' color='#ef4444'>✗ ZATVARA</font></b>", ss['Normal']),
        ]
        reg_rows = [reg_header]
        for _, r in reg_grp.iterrows():
            okrug = r['Region']
            mali = " *" if okrug in mali_okruzi else ""
            color = '#f97316' if mali else '#111111'
            reg_rows.append([
                Paragraph(f"<font size='8' color='{color}'>{okrug}{mali}</font>", ss['Normal']),
                Paragraph(f"<font size='8'><b>{int(r['Ukupno'])}</b></font>", ss['Normal']),
                Paragraph(f"<font size='8' color='#10b981'><b>{int(r['Ostaje'])}</b></font>", ss['Normal']),
                Paragraph(f"<font size='8' color='#ef4444'><b>{int(r['Zatvara'])}</b></font>", ss['Normal']),
            ])
        uk = reg_grp['Ukupno'].sum(); uk_o = reg_grp['Ostaje'].sum(); uk_z = reg_grp['Zatvara'].sum()
        reg_rows.append([
            Paragraph("<b>UKUPNO</b>", ss['Normal']),
            Paragraph(f"<b>{int(uk)}</b>", ss['Normal']),
            Paragraph(f"<font color='#10b981'><b>{int(uk_o)}</b></font>", ss['Normal']),
            Paragraph(f"<font color='#ef4444'><b>{int(uk_z)}</b></font>", ss['Normal']),
        ])
        reg_tbl = Table(reg_rows, colWidths=[W*0.55, W*0.15, W*0.15, W*0.15])
        reg_style = [
            ('BACKGROUND',(0,0),(-1,0),colors.HexColor('#f9fafb')),
            ('LINEBELOW',(0,0),(-1,0),1,colors.HexColor('#e5e7eb')),
            ('LINEABOVE',(0,-1),(-1,-1),1.5,colors.HexColor('#e5e7eb')),
            ('BACKGROUND',(0,-1),(-1,-1),colors.HexColor('#f9fafb')),
            ('TOPPADDING',(0,0),(-1,-1),3),('BOTTOMPADDING',(0,0),(-1,-1),3),
            ('LEFTPADDING',(0,0),(-1,-1),5),
            ('ALIGN',(1,0),(-1,-1),'CENTER'),
        ]
        for i in range(1, len(reg_rows)-1):
            reg_style.append(('LINEBELOW',(0,i),(-1,i),0.3,colors.HexColor('#f3f4f6')))
        reg_tbl.setStyle(TableStyle(reg_style))
        story.append(reg_tbl)
        if mali_okruzi:
            story.append(Spacer(1, 2*mm))
            story.append(Paragraph(f"* Okruzi sa manje od 5 profitabilnih objekata ({', '.join(mali_okruzi)}): Ne preporučuje se angazovanje komercijalistu isključivo za ove okruge.", s_small))
...
Ostaju samo objekti koji su u plusu.</p>
                    ...
                    st.markdown("---")
                    try:
                        pdf_buf = create_pdf(engine, period_str2, n_mes)
                        fname_pdf = f"Analiza_profitabilnosti_{datetime.date.today().strftime('%Y%m%d')}.pdf"
                        st.download_button(
                            "📄 Preuzmi PDF — Analiza profitabilnosti mreže objekata",
                            data=pdf_buf, file_name=fname_pdf,
                            mime="application/pdf", use_container_width=True)
                    except Exception as pdf_err:
                        st.warning(f"PDF nije mogao biti generisan: {pdf_err}")
