...
def create_pdf(engine):
    ...
    # Izvuci kolone
    trosak_col = [c for c in prof.columns if 'Trosak' in c][0]
    bruto_col  = [c for c in prof.columns if 'Bruto' in c][0]
    neto_col   = [c for c in prof.columns if 'Neto' in c][0]
    oos_col    = [c for c in prof.columns if 'OOS' in c and 'Izgub' in c][0]
    pot_col    = [c for c in prof.columns if 'Potencijal' in c][0]

    trosak_period_obj = float(prof[trosak_col].iloc[0])
    trosak_mes_obj    = trosak_period_obj / 6
    ukupan_trosak_6m  = prof[trosak_col].sum()
    ukupan_trosak_mes = ukupan_trosak_6m / 6

    bruto_6m = prof[bruto_col].sum()
    neto_6m  = prof[neto_col].sum()
    oos_6m   = prof[oos_col].sum()
    pot_6m   = prof[pot_col].sum()
    n_obj    = len(prof)

    neg     = prof[prof[neto_col] < 0]
    poz     = prof[prof[neto_col] > 0]
    n_neg   = len(neg)
    n_poz   = len(poz)

    neg_pot     = prof[prof[pot_col] < 0]
    n_neg_pravi = len(neg_pot)
    spaseni     = prof[(prof[neto_col] < 0) & (prof[pot_col] >= 0)]
    n_spaseni   = len(spaseni)
    poz_pot     = prof[prof[pot_col] >= 0]
    n_poz_pot   = len(poz_pot)

    usteda_6m  = n_neg_pravi * trosak_period_obj
    usteda_mes = n_neg_pravi * trosak_mes_obj

    # Mesecni trend iz originalnih prodajnih podataka
    ml = engine.analitika_labels if engine.analitika_labels else engine.mesec_labels
    mes_vals = []
    for lb in ml:
        col_name = f'Profit_{lb}'
        if col_name in prof.columns:
            mes_vals.append(float(prof[col_name].sum()))
        else:
            # Pokusaj Neto_ kolonu
            col_name2 = f'Neto_{lb}'
            if col_name2 in prof.columns:
                mes_vals.append(float(prof[col_name2].sum()))
            else:
                mes_vals.append(0.0)
    mes_labels_short = []
    mn = {1:'Jan',2:'Feb',3:'Mar',4:'Apr',5:'Maj',6:'Jun',7:'Jul',8:'Avg',9:'Sep',10:'Okt',11:'Nov',12:'Dec'}
    for lb in ml:
        parts = lb.split()
        mes_labels_short.append(parts[0] if parts else lb)

    # Ako nema mes_vals, uzmi bruto po mesecima direktno iz prodaje
    if not mes_vals or all(v == 0 for v in mes_vals):
        a_set = set((int(g), int(m)) for g, m in (engine.analitika_meseci or engine.meseci_order))
        prodaja_a = engine.prodaja[engine.prodaja.apply(
            lambda r: (int(r['Godina']), int(r['Mesec'])) in a_set, axis=1)]
        for g, m in (engine.analitika_meseci or engine.meseci_order):
            sub = prodaja_a[(prodaja_a['Godina']==g) & (prodaja_a['Mesec']==m)]
            mes_vals.append(float(sub['Profit'].sum()) if 'Profit' in sub.columns else 0.0)
            mes_labels_short.append(mn.get(int(m), str(m)))

    pad_pct = (mes_vals[-1] - mes_vals[0]) / mes_vals[0] * 100 if mes_vals and mes_vals[0] != 0 else 0

    oos_na_nuli = 0
    if len(engine.df_oos) > 0:
        oos_na_nuli = int((engine.df_oos['Lager_danas'] == 0).sum())

    neto_poz_pot_6m = float(poz_pot[neto_col].sum())
    kombinovano = neto_poz_pot_6m + usteda_6m + oos_6m
    ...
    # Period string
    if engine.analitika_labels and len(engine.analitika_labels) >= 2:
        period_str = f"{engine.analitika_labels[0]} — {engine.analitika_labels[-1]}"
        n_mes = len(engine.analitika_labels)
    else:
        period_str = "analizirani period"
        n_mes = len(engine.mesec_labels)
    ...
    # ── NASLOV ────────────────────────────────────────────────────────
    r.add(Paragraph(
        '<font name="Helvetica-Bold" size="22" color="#0D2B55">Analiza profitabilnosti</font>'
        '<font name="Helvetica" size="22" color="#2E86C1"> mreze objekata</font>',
        S('tit', align=TA_LEFT, sa=6, leading=28)))
    r.add(Paragraph(
        f'Finansijski pregled · {period_str} · {n_obj} objekata · {n_mes} meseci',
        S('sub', size=9, color=_C_SIVA, align=TA_LEFT, sa=8)))
    r.sp(0.4)
    r.add(HRFlowable(width='100%', thickness=0.5, color=_C_BORDER))
    r.sp(0.35)

    # ── KPI KARTICE ───────────────────────────────────────────────────
    r.add(KPIRow([
        ('Ukupan trosak · 6 meseci', f'{fmt(ukupan_trosak_6m)} RSD', f'{fmt(ukupan_trosak_mes)} RSD / mesec', __C_PLAVA2),
        ('Bruto profit · 6 meseci',  f'{fmt(bruto_6m)} RSD',        f'{fmt(bruto_6m/6)} RSD / mesec',        _C_AKCENT),
        ('Neto profit · 6 meseci',   f'{fmts(neto_6m)} RSD',        f'{fmts(neto_6m/6)} RSD / mesec',        _C_CRVENA),
        ('OOS izgubljen · 6 meseci', f'{fmt(oos_6m)} RSD',          f'{fmt(oos_6m/6)} RSD / mesec',          _C_NARAND),
    ], W))
    r.sp(0.4)

    # ── SEKCIJA 1: TREND ─────────────────────────────────────────────
    r.add(SectionHeader(1, 'Mesecni trend bruto profita', W))
    r.sp(0.2)
    r.add(Paragraph(
        f'Bruto profit belezi konzistentan pad — od <b>{fmt(mes_vals[0])} RSD</b> u septembru do '
        f'<b>{fmt(mes_vals[-1])} RSD</b> u februaru, sto je pad od <b>{abs(pad_pct):.0f}%</b> u samo 6 meseci. '
        f'Trosak ulistavanja je konstantan ({fmt(ukupan_trosak_mes)} RSD/mes), sto znaci da svaki mesec neto profit '
        f'sve vise ide u minus. Ovaj trend zahteva hitnu reakciju.', body_s))
    r.sp(0.2)
    r.add(BarChart(mes_labels, mes_vals, W, 3.8*cm, ''))
    r.sp(0.35)

    # ── SEKCIJA 2: PROFITABILNOST ─────────────────────────────────────
    r.add(SectionHeader(2, 'Profitabilnost po objektima', W))
    r.sp(0.2)
    ...
    tbl_data = [[
        Paragraph(
            ...
            f'Medjutim, <b>{n_spaseni} od njih ima negativan neto iskljucivo zbog OOS-a</b> — '
            f'kada se uraci izgubljena zarada, njihov potencijal je pozitivan. '
            f'Ovi objekti nisu problem, samo nisu imali robu.<br/><br/>'
            f'<b>Pravih neprofitabilnih je {n_neg_pravi}</b> ({n_neg_pravi/n_obj*100:.0f}% mreze) — '
            f'negativni cak i po potencijalu. Trosak po objektu je '
            f'<b>{fmt(trosak_period_obj)} RSD za 6m</b> / <b>{fmt(trosak_mes_obj)} RSD mesecno</b>.<br/><br/>'
            f'Zatvaranjem {n_neg_pravi} pravih neprofitabilnih: '
            f'<b>{fmt(usteda_mes)} RSD/mes</b> / <b>{fmt(usteda_6m)} RSD za 6m</b> '
            f'/ <b>{fmt(usteda_mes*12)} RSD godisnje</b>.',
            S('tl', sa=0)),
        DonutChart(n_poz_pot, n_neg_pravi, W * 0.42, 3.5*cm)
    ]]
    t = Table(tbl_data, colWidths=[W * 0.56, W * 0.44])
    ...
    # ── SEKCIJA 3: OOS ────────────────────────────────────────────────
    r.add(SectionHeader(3, 'Izgubljena zarada zbog nedostatka robe (OOS)', W))
    r.sp(0.2)

    # OOS top artikli
    oos_art = oos_df.groupby('Naziv')['Izgubljeni profit (RSD)'].sum().sort_values(ascending=False).head(5)
    oos_total = oos_df['Izgubljeni profit (RSD)'].sum()

    r.add(Paragraph(
        f'U periodu sep 2025 — feb 2026, procenjeni izgubljen profit zbog OOS iznosi '
        f'<b>{fmt(oos_6m)} RSD</b> ({fmt(oos_6m/6)} RSD/mes). Trenutno je '
        f'<b>{oos_na_nuli} kombinacija objekat-artikal na nultim zalihama</b> — direktan svakodnevni gubitak.',
        body_s))
    r.sp(0.15)

    # Top 5 OOS artikli — mini horizontalni prikaz
    r.add(Paragraph('<b>Top 5 artikala po izgubljenom profitu (Sep–Feb 2026):</b>',
                    S('hd', size=8.5, bold=True, sa=4)))

    max_oos = oos_art.values[0]
    for naziv, val in oos_art.items():
        naziv_kratki = naziv[:52] + '…' if len(naziv) > 52 else naziv
        pct = val / max_oos
        row_data = [[
            Paragraph(naziv_kratki, S('an', size=7.5, sa=0)),
            Paragraph(f'<b>{fmt(val)} RSD</b>', S('av', size=7.5, bold=True, align=TA_RIGHT, sa=0))
        ]]
        row_t = Table(row_data, colWidths=[W * 0.62, W * 0.38])
        row_t.setStyle(TableStyle([
            ('LEFTPADDING', (0,0), (-1,-1), 4),
            ('RIGHTPADDING', (0,0), (-1,-1), 4),
            ('TOPPADDING', (0,0), (-1,-1), 3),
            ('BOTTOMPADDING', (0,0), (-1,-1), 3),
            ('BACKGROUND', (0,0), (-1,-1), ___C_SIVA_LL),
    ...
    # ── SEKCIJA 4: SCENARIO ───────────────────────────────────────────
    r.add(SectionHeader(4, 'Scenario: Optimalna mreza', W))
    r.sp(0.2)
    r.add(Paragraph(
        f'Ako se istovremeno zatvore neprofitabilni objekti i eliminise OOS, '
        f'mreza ide sa <b>{fmts(neto_6m)} RSD</b> na <b>+{fmt(kombinovano)} RSD neto profita za 6 meseci</b>:',
        body_s))
    r.sp(0.2)
    ...
    kombinovano = neto_poz_pot_6m + usteda_6m + oos_6m
    r.add(ScenarioBar([
        (f'Neto profit {n_poz_pot} obj. (poz. potencijal)', neto_poz_pot_6m, _C_AKCENT, False),
        (f'Usteda: zatvaranje {n_neg_pravi} obj.',          usteda_6m,        _C_ZELENA,  False),
        ('Povracaj izgub. zarade (OOS eliminacija)',         oos_6m,           _C_NARAND,  False),
        ('UKUPNI POTENCIJAL  (6 meseci)',                   kombinovano,       _C_PLAVA,   True),
    ], W, 3.8*cm))

    r.sp(0.35)

    # ── ZAKLJUCCI ─────────────────────────────────────────────────────
    r.add(SectionHeader(5, 'Zakljucci i prioritetne akcije', W))
    r.sp(0.15)

    akcije = [
        (_C_CRVENA, '01', 'HITNO — Dopuniti zalihe',
         f'Odmah snabdeti {oos_na_nuli} kombinacija na nultim zalihama. Bez strukturnih promena, '
         f'samo redovnim snabdevanjem vracamo {fmt(oos_6m/6)} RSD profita mesecno.'),
        (_C_NARAND, '02', f'Zatvaranje {n_neg_pravi} objekata negativnog potencijala',
         f'Ovi objekti imaju negativan potencijal cak i uz punu dostupnost robe — nema ocekivanog prihoda. '
         f'Cista usteda: {fmt(usteda_mes)} RSD/mes / {fmt(usteda_mes*12)} RSD/god. '
         f'Posebno: {n_spaseni} neto-negativnih objekata ima OOS potencijal — njima treba roba, ne zatvaranje.'),
        (__C_PLAVA2, '03', 'Pracenje trenda pada prodaje',
         f'Pad od {abs(pad_pct):.0f}% za 6 meseci zahteva analizu uzroka — sezonalnost, gubitak klijenata ili OOS?'),
    ]

    for col, num, title, text in akcije:
        row = [[
            Paragraph(f'<b>{num}</b>',
                      S('n', size=16, align=TA_CENTER, sa=0, color=col)),
            [Paragraph(f'<b>{title}</b>', S('at', size=9, bold=True, sa=2, color=col)),
             Paragraph(text, S('ab', size=8.5, sa=0))]
        ]]
        rt = Table(row, colWidths=[1*cm, W - 1*cm])
    ...
    r.sp(0.15)
    r.add(Paragraph(
        f'Kombinovanjem svih mera, mreza prelazi sa trenutnog neto deficita od <b>{fmts(neto_6m)} RSD</b> '
        f'na potencijalni profit od <b>{fmt(kombinovano)} RSD za 6 meseci</b> '
        f'(<b>{fmt(kombinovano/6)} RSD mesecno</b>).',
        S('fin', size=9, color=_C_PLAVA, bold=True, align=TA_CENTER, sb=4)))
    r.add(Paragraph('', S('sp', sa=0)))
...
""")
            ...
            with dl_col2:
                if engine.has_prices:
                    if st.button("\U0001f4ca Generisi PDF izvestaj", use_container_width=True, key="gen_pdf"):
                        with st.spinner("Generisanje PDF-a..."):
                            pdf_buf = create_pdf(engine)
                        if pdf_buf:
                            st.session_state['pdf_buf'] = pdf_buf.getvalue()
                            st.session_state['pdf_fname'] = f"IZVESTAJ_AMAN_{datetime.date.today().strftime('%Y%m%d')}.pdf"
                    if 'pdf_buf' in st.session_state:
                        st.download_button(
                            f"\U0001f4e5 Preuzmi PDF — {st.session_state['pdf_fname']}",
                    ...
                    st.info("PDF nije dostupan — potrebne su kolone cena i profita.")
