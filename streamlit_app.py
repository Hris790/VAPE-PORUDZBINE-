"""
VAPE PORUDZBINE — Streamlit Web Aplikacija
"""
...
st.set_page_config(page_title="VAPE Porudzbine", page_icon="💨", layout="wide", initial_sidebar_state="expanded")
...
class PredictionEngine:
    ...
    def run(self, progress_bar):
        progress_bar.progress(5, "Ucitavanje sheetova...")
        self._load_sheets()
        progress_bar.progress(15, "Priprema podataka...")
        self._prepare_lookups()
        progress_bar.progress(25, "Racunanje povrata i korekcija...")
        self._compute_povrat_korekcija()
        progress_bar.progress(40, "Mesecni pregled...")
        self._build_monthly()
        progress_bar.progress(55, "OOS-korigovana predikcija...")
        self._predict_all()
        progress_bar.progress(70, "Spajanje sa lagerom...")
        self._merge_lager()
        progress_bar.progress(80, "Racunanje porudzbina...")
        self._compute_orders()
        progress_bar.progress(85, "Primena min pravila...")
    ...
    def _load_sheets(self):
        ...
        def find(kws):
            ...
            return None
        s_prod = find(['prodaja']); s_start = find(['startni']); s_pov = find(['povrat']); s_tl = find(['trenutni'])
    ...
    def _prepare_lookups(self):
        ...
        for _, r in self.prodaja.iterrows():
            ...
            self.prodaja_dict[key] = (r.get('Prodata Kolicina', r.get('Kolicina', 0)), r.get('Lager', 0), prom if not pd.isna(prom) else 0)
        self.povrat_total = {}
        ...
        for god, mes in self.meseci_order:
            for ida in all_art:
                ...
                for _, k in self.all_keys[self.all_keys['id artikla']==ida].iterrows():
                    idk2 = k['ID KOMITENTA']; im = implied.get((idk2, ida, god, mes), 0)
                    if im > 0: pi[idk2] = im
                    elif im < 0: ni[idk2] = im
                tp = sum(pi.values())
                if ap > 0 and tp > 0:
                ...
                for i, v in ni.items():
                    self.final_korekcija[(i, ida, god, mes)] = self.final_korekcija.get((i, ida, god, mes), 0) + v
    ...
    def _predict_all(self):
        ...
        for _, k in self.all_keys.iterrows():
            ...
            analysis.append({'idk': idk, 'ida': ida, 'sales': np.array(sales, dtype=float),
                             'oos': np.array(oos), 'poc': np.array(pocs, dtype=float), 'll': lv})
        preds = {}
        for it in analysis:
            s, o, p = it['sales'], it['oos'], it['poc']; n = len(s)
            noos = s[o == 0]
            ...
            if n >= 2:
                ...
                for i in range(1,n):
                    ...
                    nt = self.beta*(nl-lev)+(1-self.beta)*tr; lev, tr = nl, nt
                holt = lev+tr
            else: holt = adj[0]; tr = 0
        ...
        for ida in df_p['k'].apply(lambda x: x[1]).unique():
            ...
            if d != 0:
                rem = sub['p']-np.floor(sub['p']); adj_m = sub['p']>0
                if d > 0:
                    for idx in rem[adj_m].sort_values(ascending=False).index[:int(d)]: df_p.loc[idx,'pr'] += 1
                elif d < 0:
                    for idx in rem[adj_m&(sub['pr']>0)].sort_values(ascending=True).index[:int(abs(d))]:
...
def create_excel(engine):
    ...
    sf = Font(bold=True, name='Arial', size=9); dfn = Font(name='Arial', size=9)
    ...
    ord_hdr = PatternFill('solid', fgColor='375623')
    SC = 5; sub_h = ['Pocetno stanje','Promet (ulaz)','Prodaja','Povrat','Korekcija']
    sub_f = [sf_poc, sf_prom, sf_prod, sf_pov, sf_kor]
    col_suf = ['_Pocetno','_Promet','_Prodaja','_Povrat','_Korekcija']
    ...
    for i, label in enumerate(ml):
        ...
        for j, (sh, sfill) in enumerate(zip(sub_h, sub_f)):
            cell = ws1.cell(2, sc+j, sh); cell.font = sf; cell.fill = sfill; cell.border = tb; cell.alignment = caw
    ps = 5 + len(ml)*SC
    ws1.merge_cells(start_row=1, end_row=1, start_column=ps, end_column=ps+2)
    cell = ws1.cell(1, ps, f'{engine.pred_label} - PREDIKCIJA')
    cell.font = Font(bold=True, color='FFFFFF', name='Arial', size=10); cell.fill = pred_hdr; cell.alignment = ca
    ...
    for j, (sh, sfill) in enumerate(zip(['Predikcija','Prosek (5m)','Razlika'], [sf_pred, sf_avg, sf_razl])):
        cell = ws1.cell(2, ps+j, sh); cell.font = sf; cell.fill = sfill; cell.border = tb; cell.alignment = caw
    os_c = ps + 3
    ws1.merge_cells(start_row=1, end_row=1, start_column=os_c, end_column=os_c+2)
    cell = ws1.cell(1, os_c, f'PORUDZBINA - {engine.order_label}')
    cell.font = Font(bold=True, color='FFFFFF', name='Arial', size=10); cell.fill = ord_hdr; cell.alignment = ca
    for cc in range(os_c, os_c+3): ws1.cell(1, cc).border = tb; ws1.cell(1, cc).fill = ord_hdr
    lager_label = "Lager danas"
    if len(engine.trenutni) > 0:
        dan_col = [c for c in engine.trenutni.columns if 'dan' in c.lower()]
        if dan_col:
            try:
                d = pd.to_datetime(engine.trenutni[dan_col[0]].iloc[0])
                lager_label = f"Lager na dan\n{d.strftime('%d.%m.%Y')}"
            except: pass
    for j, (sh, sfill) in enumerate(zip([lager_label, 'Porudzbina\n(osnovna)', f'Porudzbina\n(min. {engine.min_lager} na stanju)'], [sf_lager, sf_p1, sf_p2])):
        cell = ws1.cell(2, os_c+j, sh); cell.font = sf; cell.fill = sfill; cell.border = tb; cell.alignment = caw
    for idx, row in df.iterrows():
        ...
        for i, label in enumerate(ml):
            ...
            for j, suf in enumerate(col_suf):
                ...
                cell = ws1.cell(r, cb+j, int(v) if not pd.isna(v) else 0)
                cell.font = dfn; cell.alignment = Alignment(horizontal='center'); cell.border = tb
        for j, cn in enumerate(['Predikcija','Prosek','Razlika']):
            v = int(row.get(cn, 0)); cell = ws1.cell(r, ps+j, v)
        ...
        for j, cn in enumerate(['Lager_danas','Porudzbina_1','Porudzbina_2']):
            v = int(row.get(cn, 0)); cell = ws1.cell(r, os_c+j, v)
            cell.alignment = Alignment(horizontal='center'); cell.border = tb
            if cn != 'Lager_danas' and v > 0: cell.font = Font(name='Arial', size=9, bold=True, color='375623')
            else: cell.font = dfn
    ws1.column_dimensions['A'].width = 14; ws1.column_dimensions['B'].width = 11
    ws1.column_dimensions['C'].width = 50; ws1.column_dimensions['D'].width = 12
    for i in range(len(ml)):
        for j in range(SC): ws1.column_dimensions[get_column_letter(5+i*SC+j)].width = 14
    ...
    for r, label in enumerate(ml, 2):
        ws2.cell(r,1,label).font = Font(bold=True, name='Arial', size=10); ws2.cell(r,1).alignment = ca; ws2.cell(r,1).border = tb
        ...
        for c2, (v, f) in enumerate(zip(vals, fills), 2):
            cell = ws2.cell(r, c2, v); cell.font = Font(name='Arial', size=10); cell.fill = f; cell.alignment = ca; cell.border = tb; cell.number_format = '#,##0'
    fr = len(ml) + 3
    ...
    info_rows = [(f'Predikcija prodaje {engine.pred_label}', int(df['Predikcija'].sum()), sf_pred),
        ('Prosek (5m)', int(df['Prosek'].sum()), sf_avg),
        ...
        (f'Porudzbina (min. {engine.min_lager} na stanju)', int(df[~df['ID KOMITENTA'].isin(engine.excluded)]['Porudzbina_2'].sum()), sf_p2)]
    for i, (label, val, fill) in enumerate(info_rows, fr+1):
    ...
    ws3 = wb.create_sheet("O modelu predikcije"); ws3.column_dimensions['A'].width = 100
    info = ["OPIS MODELA PREDIKCIJE I PORUDZBINE", "",
        f"=== PREDIKCIJA PRODAJE ZA {engine.pred_label.upper()} ===", "",
        "Model predvidja POTENCIJAL PRODAJE - koliko bi svaki objekat prodao kada bi imao robu.", "",
        f"  1. OOS korekcija - meseci bez zaliha se ne broje kao 0 prodaja",
        f"  2. Holt DES (alpha={engine.alpha}, beta={engine.beta}) + WMA",
        "  3. Parcijalni OOS: blend 50/50 ako stanje < 50% prosecne prodaje",
        "  4. Zaokruzivanje na cele brojeve (largest remainder)", "",
        f"=== PORUDZBINA ZA {engine.order_label.upper()} ===", "",
        f"Osnovna: max(Predikcija - Lager, 0)",
        f"Min {engine.min_lager}: max(Predikcija - Lager, {engine.min_lager} - Lager, 0)",
        f"Min po objektu: >= {engine.min_order} ili 0",
        f"Iskljuceni: {', '.join(str(x) for x in sorted(engine.excluded))}", "",
        f"Generisano: {datetime.datetime.now().strftime('%d.%m.%Y. u %H:%M')}"]
...
def main():
    st.markdown("""<div class="header-banner"><div class="header-title">💨 VAPE PORUDZBINE</div>
    ...
    uploaded = st.file_uploader("Ucitaj Excel fajl (prodaja, startni lager, povrat, trenutni lager)", type=['xlsx','xls'])
    if uploaded:
        ...
        if st.button("POKRENI - Generisi porudzbinu", use_container_width=True):
            ...
            try:
                ...
                with tab1:
                    show = result[['ID KOMITENTA','id artikla','Naziv artikla','Grupa','Predikcija','Prosek','Lager_danas','Porudzbina_1','Porudzbina_2']].copy()
                    show.columns = ['ID Kom.','ID Art.','Naziv','Grupa','Pred.','Prosek','Lager','P1','P2']
        ...
        st.markdown("""<div style="text-align:center;padding:60px 20px;color:#aaa;">
            ...
            <div style="font-size:16px;color:#888;">Ucitaj Excel fajl da pocnes</div></div>""", unsafe_allow_html=True)
